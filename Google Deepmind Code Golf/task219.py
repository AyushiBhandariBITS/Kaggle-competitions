def p(x):
 ...
 for(d,o)in t:t=k(o);x=c(e,t);x=[set(c(x,o)[1])for o in y];d=set(o);p=[(o,len(o&d),min(o for(o,a)in o)if o else u('inf'))for o in x];m=max(p,key=lambda k:(k[1],-k[2]));g=max(o for(a,o)in o);b.append((m[0]if m[1]>0 else d,g))
 i=[o[:]for o in a]
 for(p,j)in b:
  for(y,r)in p:
   if r<=j:continue
